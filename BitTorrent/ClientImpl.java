...
public class ClientImpl extends UnicastRemoteObject implements ClientI {
    ...
    ServerI IndexServer; // Connectio with the TrackerServer
    ...
    ConcurrentMap<String, Progreso> progreso; // with this, the peers can communicate with each other and know who has
                                              // which file
    ...
    public ClientImpl(ServerI IndexServer, PeerData p) throws RemoteException {
        ...
        dirname = instanceName; // el nombre del directorio. Se manejo con otra variable por si en un futuro se
                                // cambiaba la direccion de este
        dir = new File(dirname);// the name of the directory. It was handled with another variable in case in
                                // the future the address of the directory was changed.
     ...
     * @return
     */
    public boolean run() {// the main method
        // main UI loop
    ...
    public byte[] obtain(String file, int piece) {// is a remote function, it returns an array of bytes
        ...
        if (inConnections < 15)
            inConnections++;// check the limit of connections, since 15 is the maximum.
        else
            return null;
        ...
        if (!readfile.exists()) { // The fragment does not exist
            ...
            if (!file2frag.exists()) {// does not exist returns null
                inConnections--;
                return null; // if it does not exist, returns null
            }
            ...
            frg.fragmentar(pathfile2frag, instanceName + "/", PIECE_LENGTH);// in case the file is larger than 512KB, it
                                                                            // fragments it.
        }
        bytes = frg.read(readfile); // now yes, read the fragment to return it
        inConnections--;
    ...
    private void search(String filename)
        ...
        double startTime = System.nanoTime();// is used to count the response time
        double endTime;
    ...
    private void register(File dir) { // records all files in the directory
        ...
        try {
            IndexServer.registryPeer(p); // the peer is registered in case for some reason its registration was lost on
                                         // the server side.
        ...
        try {
            ...
            for (int i = 0; i < sharedfiles.length; i++) {
                ...
                if (!sharedfiles[i].getName().endsWith(".bin")) { // makes a filter to avoid recording fragments
                    ...
                    Progreso prg = new Progreso(tmp.getName(), tmp.getPieces()); // you create a progress object
    ...
    public Progreso getProgress(String filename) {
        return progreso.get(filename); // scrambles the progress related to the indicated file
    }
    ...
    private void getFile(String filename)// this is the main function that is in charge of obtaining the files
        ...
        int[] connection = new int[4];// this is to know the Id's of the peers you are already connected to (moreover,
                                      // it allows 4 connections per file)
        ...
        seeders.remove(p);// removes the peer that wants to get the file from the list to avoid connecting
                          // to itself
        if (seeders.size() < 1) {
            System.out.println("There are not enough seeders");
            return;
        ...
        if (prg.getPiecesLength() == 1) {// is 512KB or smaller
            HiloP hilo;
        ...
        Iterator<PeerData> it = seeders.iterator();// an iterator is used, because it will be interrupted and then
                                                   // continued.
        ...
        leechers.remove(p);// eliminates itself as a leecher (this is a precaution, in theory it should not
                           // to be on the list unless you exit the program and run it again).
    ...
    public void unir(String filename, int piezas) {// joins the fragments
